                ...
                script {
                    sshagent (credentials: ['airtracking-runtime']) {
                        ...
                        if (env.APPLICATION == 'API & WebApp & Stats' || env.APPLICATION == 'WebApp') {
                            ...
                            sh 'ssh -o StrictHostKeyChecking=no -l esp52 192.168.160.103 docker run -d -p 9070:8080 --network="esp52" --name esp52-airtrackingwebapp 192.168.160.99:5000/esp52-airtrackingwebapp'
                        }
                    }
                }
            }
